...
RSpec.describe TypedEAV::Config do
  ...
  describe "#field_class_for" do
    it "resolves type name to class" do
      expect(config.field_class_for(:integer)).to eq(TypedEAV::Field::Integer)
    end
  ...
  describe "#register_field_type" do
    ...
    it "registers a custom field type" do
      config.register_field_type(:custom_test, "TypedEAV::Field::Text")
      expect(config.type_names).to include(:custom_test)
      expect(config.field_class_for(:custom_test)).to eq(TypedEAV::Field::Text)
    end
  end
  ...
  describe "#type_names completeness" do
    ...
    it "resolves all 17 builtin types" do
      TypedEAV::Config::BUILTIN_FIELD_TYPES.each_key do |name|
...
RSpec.describe TypedEAV::Registry do
  ...
  describe "#type_allowed?" do
    ...
    it "allows any type when unrestricted" do
      expect(registry.type_allowed?("Contact", TypedEAV::Field::Json)).to be true
    end

    it "allows permitted types" do
      expect(registry.type_allowed?("Product", TypedEAV::Field::Integer)).to be true
    end

    it "rejects non-permitted types" do
      expect(registry.type_allowed?("Product", TypedEAV::Field::Json)).to be false
    end
  end
